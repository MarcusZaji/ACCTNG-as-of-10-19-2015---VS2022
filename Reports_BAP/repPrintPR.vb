...
Partial Public Class repPrintPR
    ...
    Public Sub New()
        ...
        Try
            ...
            Dim myDataInvestment As DataTable = Chinita.fetchDBTable("select tr1.fldInvestmentNo, " _
                                                           & "(select tr2.fldProdCostJPY from tblInvestment tr2 where tr2.fldInvestmentNo = tr1.fldInvestmentNo) - " _
                                                           & "(select sum(ifnull(tr2.fldEJPY,0))  from tblInvestmentRecord tr2 where tr2.fldInvestmentNo  = tr1.fldInvestmentNo  AND tr2.fldStatus is null) as InvestmentBal " _
                                                           & "from tblInvestmentRecord tr1 where tr1.fldPRNo  = '" & PR & "'")

            Dim TotalInvestmentBal As Decimal = Convert.ToDecimal(myDataInvestment.Rows(0).Item("InvestmentBal").ToString)

            Dim InvestmentNo As String = myDataInvestment.Rows(0).Item("fldInvestmentNo").ToString

            'txtInvestmentBal.Value = 
            txtInvestmentBal.Value = String.Format("{0:N2}", TotalInvestmentBal)
            txtInvestmentNo.Value = InvestmentNo
        Catch ex As Exception
            txtInvestmentBal.Value = "N/A"
            txtInvestmentNo.Value = "N/A"
        End Try
        '---------------------------------------------------------------------------------
